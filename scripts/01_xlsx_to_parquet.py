...
FIELDS = ['שנה','חודש','מחלקה','קטגוריה','ספק','יצרן',
          'מכר כספי (מיליוני ₪)',
          ...
          'מחיר ממוצע לליטר']
NF = len(FIELDS)
TEXT = set(range(0,6))            # dimension columns stay strings
...
SCHEMA = pa.schema(
    ...
    [pa.field(FIELDS[i], pa.string()) for i in range(2,6)] +
    [pa.field(FIELDS[i], pa.float64()) for i in range(6,NF)] +
...
def flush(w, cols, src):
    ...
    for i in range(2,6): arr.append(pa.array(cols[i], pa.string()))
    for i in range(6,NF): arr.append(pa.array([f2(x) for x in cols[i]], pa.float64()))
