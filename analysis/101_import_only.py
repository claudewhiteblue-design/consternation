...
ex=pd.read_csv('/tmp/category_exposure.csv').rename(columns={'ctg':'cat'})[['cat','simple_score','complex_score']].drop_duplicates('cat')
...
print('correlation between the two import measures: %.3f'%
      raw.groupby('cat')[['simple_score','complex_score']].first().corr().iloc[0,1])

PH={'כל התקופה':('2000-01','2099-12'),'פיחות 2/22-10/23':('2022-02','2023-10'),'ייסוף 11/23-7/26':('2023-11','2026-07')}
MEAS=[('מדד פשוט (נמוך/בינוני/גבוה)','simple_score'),('מדד מורכב (יצרן×קטגוריה)','complex_score')]
...
for exmeat in [False,True]:
    ...
    for nm,col in MEAS:
        ...
        print()
pd.DataFrame(SUMM).to_csv('import_only_summary.csv',index=False)
pd.DataFrame(PATHS).to_csv('import_only_paths.csv',index=False)
json.dump(dict(paths=[{k:(round(v,4) if isinstance(v,float) else v) for k,v in r.items()} for r in PATHS],
               summary=SUMM,months=MONTHS),open('import_only.json','w'),ensure_ascii=False,default=float)
print('\nby phase, ייבוא בלבד:')
