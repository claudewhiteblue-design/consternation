...
for tag,col in [('CR3 עם מאגדים','cr3_in'),('HHI','hhi'),('CR3 ללא מאגדים','cr3_ex')]:
    ...
    for mo,y in x.groupby('month'):
        ...
        rows.append(r)
    path=pd.DataFrame(rows).sort_values('month'); path['gap']=path.q4-path.q1
    ...
    cats={int(q):[dict(cat=r.cat,dep=r.dep,m=round(r.m,1),rev=round(r.rev,1))
          ...
          for q,z in g.groupby('q') if q in (1,4)}
    out[tag]=dict(path=path.round(3).to_dict('records'),quartiles=qs,cats=cats,
                  months=sorted(d.month.unique()))
    print(f'\n{"="*80}\n{tag} — רבעונים שווי מכר 2022, בסיס = ממוצע 2022\n{"="*80}')
    for z in qs: print(f'  רבעון {z["q"]}: {z["n"]:>3} קטגוריות  {z["rev"]:>8,.0f} מ׳ ₪ ({z["share"]:>4.1f}%)  '
    ...
    print(f'  ממוצע 2022 (בדיקה): q1={a.q1.mean():.1f} q4={a.q4.mean():.1f}  |  '
          f'7/2026: q1={path.q1.iloc[-1]:.1f} q4={path.q4.iloc[-1]:.1f}  פער {path.gap.iloc[-1]:+.1f}')
json.dump(out,open('conc_quartiles.json','w'),ensure_ascii=False,default=float)
