...
cc=pd.read_csv('/home/user/consternation/subcategory_concentration_2022.csv')[['sub','cat','dep','cr3_in','cr3_ex','hhi']].dropna().rename(columns={'sub':'sc'})
...
json.dump(dict(ter=ter,xrows=xrows,res2={k:{a:list(b) for a,b in v.items()} for k,v in res2.items()},
    ...
    open('asymmetry.json','w'),ensure_ascii=False)
print('\nsaved asymmetry.json / asymmetry_stats.csv')
