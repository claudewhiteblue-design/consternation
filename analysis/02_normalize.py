...
c.execute(f'''CREATE VIEW base AS
  SELECT "שנה", "חודש", CAST(strptime("חודש",'%Y/%m') AS DATE) AS period,
         "מחלקה","קטגוריה","ספק","יצרן", {R},
         {norm(U,PU)} AS {U}, {norm(T,PK)} AS {T}, {norm(L,PL)} AS {L},
         {PU}, {PC}, {PK}, {PL}, source_file
  FROM {SRC}''')
...
c.execute(f'''
COPY (
  SELECT f."שנה", f."חודש", f.period, f."מחלקה", f."קטגוריה", f."ספק", f."יצרן",
         f.{R}, f.{U}, f.{T}, f.{L}, f.{PU}, f.{PC}, f.{PK}, f.{PL},
         ...
         f.source_file
  FROM filt f JOIN basis b ON b.cat = f."קטגוריה"
  ORDER BY f.period, f."מחלקה", f."קטגוריה", f."ספק", f."יצרן"
...
c.execute(f'''COPY (
  SELECT f."קטגוריה" AS "קטגוריה", any_value(f."מחלקה") AS "מחלקה",
    CASE b.basis WHEN 'TON' THEN 'ק"ג' WHEN 'LITRE' THEN 'ליטר'
                 WHEN 'UNIT' THEN 'יח'' באריזה' ELSE 'מחזור' END AS "בסיס מדידה",
  ...
  FROM filt f JOIN basis b ON b.cat=f."קטגוריה" GROUP BY f."קטגוריה", b.basis ORDER BY 5 DESC
) TO 'out/category_measure_map.csv' (FORMAT CSV, HEADER);''')
print("built out/category_measure_map.csv")
