...
MEAS=__import__('sys').argv[1] if len(__import__('sys').argv)>1 else 'cr3_ex'
med=cc[MEAS].median()
d['grp']=np.where(d[MEAS]>=med,'high','low')
d['w']=d.cat.map(rev22)
print(f'median {MEAS} = {med:.1f}')
for g,x in d[d.month=='2022-01'].groupby('grp'):
    print(f'  {g:5} n={len(x):>4} cats  CR3 mean={x[MEAS].mean():5.1f}  2022 rev={x.cat.map(rev22).sum():>9,.0f} M')
rows=[]
...
out.to_csv(f'median_split_price_paths_{MEAS}.csv',index=False)
...
json.dump(out.round(3).to_dict('records'),open(f'median_split_paths_{MEAS}.json','w'),ensure_ascii=False)
