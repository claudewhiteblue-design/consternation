...
print(f'ללא בשר ועוף | CR3 עם מאגדים, מדידת 2022 | {d.cat.nunique()} קטגוריות x {len(months)} חודשים, n={len(d):,}, Kish={kish:.0f}')
...
T=pd.get_dummies(d.month,prefix='t',drop_first=True).astype(float)
def fit(terms,weighted):
    ...
    for lab in terms:
        ...
        path[lab]=[dict(month=m,b=100*r.params[i],se=100*r.bse[i]) for m,i in zip(months[1:],idx)]
...
pd.DataFrame(rows).to_csv('conc_with_fx_control_summary.csv',index=False)
json.dump(dict(rows=rows,paths=paths,months=months,n=int(d.cat.nunique()),kish=round(float(kish))),
          open('conc_with_fx_control.json','w'),ensure_ascii=False,default=float)
