...
def xreg(y,ctrl_sd,weighted):
    ...
    X=pd.DataFrame({'const':1.0,'cr3_z':S.z,'g20':S.g20})
    ...
    return {k:(float(r.params[k]),float(r.bse[k]),float(r.pvalues[k])) for k in ['cr3_z','g20']}
X1=[]
...
def run_panel(pos,neg,tag,fe_time):
    ...
    for weighted in [True,False]:
        ...
        X=pd.concat(base+[pd.DataFrame({'P':pos,'N':neg,'Pz':pos*long.z,'Nz':neg*long.z,
                                        'Pg':pos*long.g,'Ng':neg*long.g},index=long.index)],axis=1)
        X=sm.add_constant(X)
        r=(sm.WLS(long.dp,X,weights=long.w) if weighted else sm.OLS(long.dp,X)).fit(
            cov_type='cluster',cov_kwds={'groups':long.cat})
        o={k:(float(r.params[k]),float(r.bse[k]),float(r.pvalues[k])) for k in ['P','N','Pz','Nz','Pg','Ng']}
        nn=list(X.columns)
        for lab,a,b in [('diff_z','Pz','Nz'),('diff_g','Pg','Ng')]:
    ...
    for nm,o in out.items():
        ...
        print(f'     CR3    γ+={o["Pz"][0]:+.4f} (p={o["Pz"][2]:.3f})  γ-={o["Nz"][0]:+.4f} (p={o["Nz"][2]:.3f})  '
...
json.dump(dict(level=LEVEL,n=len(us),negm=negm,nmonths=len(dm_),lost=round(100*float(lost),1),
    ...
    open(f'asym_giants_{LEVEL}.json','w'),ensure_ascii=False)
print(f'\nsaved asym_giants_{LEVEL}.json')
